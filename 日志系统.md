...
##### 日志更新策略
1、webWorker处理先批量存储内存
2、防抖每两秒将内存更新到indexDB
3、每100条信息存储一次到indexDB
4、超时过一天的时间删除，用时间做主键
5、日志序号，超过10000条删除
```
  add(data: T) {
    ...
    // 两秒钟后一次
    this.debounceAdd()?.catch(undefined)
    // 每1000次，触发一次
    if (this.queueList.length % 100 === 0) this._add().catch(undefined)
  }
  deleteByPreserve = async (preservedDay = this.preservedDay) => {
    const db = await this.db
    if (!db) return Promise.reject('deleteByPreserve error')
    const objectStore = this.getTransaction(db, 'readwrite')
    return new Promise((resolve, reject) => {
      // const request = objectStore.openCursor()
      const preservedTime = Date.now() - preservedDay * 24 * 60 * 60 * 1000

      const nowIndex = objectStore.index('now_idx')
      // 查找过时的键
      const request = nowIndex.getKey(IDBKeyRange.lowerBound(preservedTime))

      request.onsuccess = async () => {
        const id = request.result
        if (id) await this.delete(IDBKeyRange.upperBound(id))
        resolve(null)
      }
      request.onerror = (ev) => {
        console.error('删除异常', ev)
        reject(null)
      }
    })
  }

    deleteByCountMax = async () => {
    const lastCursor = await this.findLastCursor()
    const count = (await this.getCount()) as number
    if (count <= this.maxCount) return
    const lastKey = lastCursor?.key as number - this.maxCount
    if (lastKey < 0) return
    const range = IDBKeyRange.upperBound(lastKey, true)
    await this.delete(range)
  }
```